...
export const renderRepeatButton = (mode: RepeatMode, onToggle?: EventHandler<RepeatMode>) => {
  switch (mode) {
  case RepeatMode.OFF:
    return <IconButton className={cls(style.control, style.inactiveControl)} onClick={() => callHandler(onToggle, RepeatMode.ALL)}>{RepeatIcon}</IconButton>;
  case RepeatMode.ALL:
...
export const renderShuffleButton = (mode: ShuffleMode, onToggle?: EventHandler<ShuffleMode>) => {
  ...
  case ShuffleMode.OFF:
    return <IconButton className={cls(style.control, style.inactiveControl)} onClick={() => callHandler(onToggle, ShuffleMode.ALL)}>{ShuffleIcon}</IconButton>;
